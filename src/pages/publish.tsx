...
import { Center, Heading, Flex, Text, HStack, VStack, Link, Box, Button, Tabs, TabList, Tab, TabPanel, TabPanels } from '@chakra-ui/react'
import { useState, useEffect } from 'react';
import * as Yup from 'yup';

const Publish = () => {
  const { connectWallet, isConnected, displayName, disconnectWallet } = useWalletContext();
  ...
  useEffect(() => {
    if (isConnected && libraryContract) {
      libraryContract.hasValidPublishAccess()
      .then((result) => {
  ...
  return (
    ...
    >
      <Box mb="50px">
        <Heading as='h1' size={{base: '3xl', md: '4xl'}}>Publish to the Librarium</Heading>
      ...
      {isConnected && (
          ...
          <Tabs>
            ...
            <TabPanels bgColor="#090909" py="10px" px="20px">
              <TabPanel>
                <Formik
                  ...
                  validationSchema={Yup.object({
                    ...
                    authorWallet: Yup.string()
                      .matches(/^0x([A-Fa-f0-9]{40})$/,'Must be a valid wallet address')
                      .required('Required'),
                    content: Yup.string()
                      .required('Required'),
                  })}
                  onSubmit={(
                    ...
                    setTimeout(async () => {
                      ...
                      if (confirm('Are you sure you want to record \''+ values.title + '\' by ' + values.author + '?')) {
                        console.log ('here');
                        libraryContract.record(values.title, values.author, values.authorWallet, values.content, values.tags)
                        .then((result) => {
                          ...
                          if(err.message.includes("User denied")) {
                            setSubmitRecordResult("Error: User rejected transaction.");
                          } else {
                            setSubmitRecordResult("Error: Transaction Failed!");
                          }
                          setSubmitting(false);
                        });
                      } else {
                ...
                <Box mb="15px" w={{base: '300px', md: '600px', lg: '800px'}}>
                  <label htmlFor="authorWallet">Author's Wallet</label>
                  <InputControl
                    id="authorWallet"
                    name="authorWallet"
                    inputProps={{placeholder:"0x0000000000000000000000000000000000000000"}}
                  />
                </Box>
                <Box mb="15px" w={{base: '300px', md: '600px', lg: '800px'}}>
                ...
                <HStack>
                  <Button type="submit" disabled={isSubmitting}>Record</Button>
              ...
              <TabPanel>
                <Formik
                  ...
                  validationSchema={Yup.object({
                    transactionHash: Yup.string()
                      .matches(/^0x([A-Fa-f0-9]{64})$/,'Must be a valid transaction hash')
                      .required('Required')
                  })}
                  onSubmit={(
                    values: RevokeValues,
                    ...
                    setTimeout(async () => {
                      ...
                      if (confirm('Are you sure you want to revoke \''+ values.transactionHash + '\'?')) {
                        console.log ('here');
                        libraryContract.revoke(values.transactionHash)
                        .then((result) => {
                          console.log ('result:',result);
                          ...
                          console.log(err);
                          if(err.message.includes("User denied")) {
                            setSubmitRevokeResult("User rejected transaction.");
                          } else {
                ...
                <HStack>
                  <Button type="submit" disabled={isSubmitting}>Revoke</Button>
        ...
        : (
          <Heading textAlign="center" py="40px" w={{base: '250px', md: '600px'}}>You need a write pass to publish the library. Unfortunately you don't have one.</Heading>
        )}
        </VStack>
      )}
      {!isConnected && (
