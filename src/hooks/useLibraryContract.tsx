...
if (!process.env.NEXT_PUBLIC_LIBRARY_CONTRACT_ADDRESS) 
  throw new Error("Please set the NEXT_PUBLIC_LIBRARY_CONTRACT_ADDRESS environment variable");
...
export function useLibraryContract() {
  ...
  async function record(title:String, author:String, authorWallet:String, content:String, tags:Array<String>) {
    try {
      const result = await libraryContract.record(title, author, authorWallet, content, tags);
      return result;
    } catch (err) {
      console.log(err);
      return false;
